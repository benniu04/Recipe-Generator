<script>
    ...
    onMount(() => {
      ...
      async function fetchOpenAIRecipe() {
        ...
        try {
          const response = await fetch('http://localhost:4000/api/generate-recipe', { // Ensure this matches your server's port
            method: 'POST',
          ...
          recipeDisplay.innerHTML = `<p class="result-recipes">${recipe}</p>`;
          console.log("Fetched Recipe", data);
        } catch (error) {
          console.error('Error fetching recipe:', error);
          recipeDisplay.innerHTML = `<p>Error fetching recipes. Please try again later.</p>`;
        } finally {
      ...
      window.saveRecipe = async (recipeID, button) => {
        ...
        try {
          ...
          // Update the UI with styled elements
