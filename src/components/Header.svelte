<script>
    ...
    onMount(() => {
      ...
      async function fetchOpenAIRecipe() {
        const prompt = `Give me ${numOfRecipes} recipes based on the ingredients ${inputIngredients} and a dietary preference of ${dietaryPreference}. 
        ...
        try {
            const response = await fetch('http://localhost:4001/api/generate-recipe', {
                method: 'POST',
            ...
            const recipes = data.recipe.split('RECIPE_START').filter(r => r.trim()).map(recipe => {
                // Clean up the recipe text
                recipe = recipe.split('RECIPE_END')[0];
                recipe = recipe.replace(/\b(Enjoy|Hope you enjoy|Bon appétit).*$/i, '');
                return recipe.trim();
            });
            ...
            recipeDisplay.innerHTML = `
                <div class="result-recipes">
                    ${recipes.map((recipe, index) => {
                        // Extract the recipe name and details
                        const [title, ...details] = recipe.split('\n').filter(line => line.trim());
                        return `
                            <div class="recipe-card" data-recipe-index="${index}">
                                <strong>${title}</strong>
                                <div class="recipe-details">
                                    ${details.join('<br>').replace(/\n/g, '<br>')}
                                </div>
                                <div class="button-container">
                                    <button class="save-button" onclick="window.handleSaveRecipe(${index}, this)">
      ...
      window.handleSaveRecipe = async (recipeID, button) => {
        if (!user) {
        ...
        try {
            ...
            const recipeData = {
                title: titleElement.textContent.replace(/^Recipe #\d+:\s*/, '').trim(),
                                ...
                                <div class="how-container">
                                    <div class="step">
                                        <div class="step-content">
                                            ...
                                            <p class="pl-10 pt-5 pb-10"> → Enter however many ingredients you want!</p>
                                        </div>
                                    ...
                                    <div class="step">
                                        <div class="step-content">
                                            <h2 class="text-xl pt-10 pl-10">Enter Preference</h2>
                                            <p class="pl-10 pt-5"> → Enter your dietary preferences for customized options.</p>
                                            <p class="pl-10 pt-5 pb-10"> → This includes keto, vegetarian, gluten-free, etc.</p>
                                        </div>
                                    ...
                                    <div class="step">
                                        <div class="step-content">
                                            <h2 class="text-xl pt-10 pl-10">Enter the number of recipes you want</h2>
                                            <p class="pl-10 pt-5 pb-10"> → Try to keep the number below 10!</p>
                                        </div>
                ...
                <div class="about-contain-1">
                    <div class="gen-box">
                        <div id="recipe" class="actual-input-box">
                            ...
                            <div class="input-ing">
                                <input type="text" id="diet-pref" placeholder="Enter your dietary preference...">
                            </div>
                            <div class="input-ing">
                        ...
                        <div class="output-box" id="recipe-display">
